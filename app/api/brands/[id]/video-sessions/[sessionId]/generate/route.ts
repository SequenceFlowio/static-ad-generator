...
async function runVideoGeneration(sessionId: string, seedancePrompt: string, referenceImageUrls: string[], aspectRatio: string, duration: number) {
...
// POST — kick off Seedance 2 video generation
export async function POST(
  ...
  // Build reference images: scene frames in order + product image last
  ...
  if (!videoSession.seedance_prompt) {
    return NextResponse.json({ error: "No Seedance prompt set" }, { status: 400 });
  }
