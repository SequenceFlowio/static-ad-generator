...
import type { FacebookConnection, FbAdInsights } from "@/types";
...
function AnalyticsPageInner() {
  ...
  useEffect(() => {
    ...
    if (errorParam) console.error("FB OAuth error:", errorParam);
    const url = new URL(window.location.href);
    if (url.searchParams.has("connected") || url.searchParams.has("brand_id") || url.searchParams.has("error")) {
  ...
  // Auto-load when brand changes
  useEffect(() => {
    if (!brand) { setConnection(null); setInsights([]); setTotals(null); return; }
    setInsights([]);
    setTotals(null);
    loadConnection(brand.id);
    loadInsights(brand.id, datePreset);
  ...
  function handleVariation(insight: FbAdInsights) {
    ...
    router.push(`/generation/ads?ref_image=${encodeURIComponent(insight.creative_image_url)}`);
  }
    ...
    return (
      <div className="min-h-screen px-4 py-8 max-w-7xl mx-auto">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Selecteer een brand via het menu linksonder om analytics te bekijken.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Link
          href="/meta"
          className="flex items-center gap-1.5 rounded-lg border border-gray-200 dark:border-gray-700 px-3 py-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 hover:border-[#1877F2] hover:text-[#1877F2] transition-colors"
        >
